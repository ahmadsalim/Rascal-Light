...
PhpExpr toPhpExpr(Expression expr) {
	switch(expr) {
		...
		case float(real r): return phpScalar(phpFloat(r)); 
		...
		case \list(list[Expression] items): {
		 ...
		 return phpNew(phpName(phpName("Vector")), 
		...
		case \map(map[Expression key, Expression \value] m): {
			...
			for (k <- m) 
				elements = elements + 
							[phpActualParameter(phpNew(phpName(phpName("Pair")), 
					        	[phpActualParameter(toPhpExpr(k), false), 
					        		phpActualParameter(toPhpExpr(m[k]), false)]), false)];
		    return phpStaticCall(phpName(phpName("MapFactory")), phpName(phpName("createFromPairs")), elements);
		}
		
		case get(artifact(str name)):
			return phpPropertyFetch(phpVar(phpName(phpName("this"))), phpName(phpName("_" + toLowerCaseFirstChar(name))));
		
		case variable(str name):
			return phpVar(phpName(phpName(name)));
		...
		case toPhpExpr(new(str artifact, list[Expression] args)): {
			list[PhpActualParameter] phpParams = [];
			for (arg <- args)
				phpParams = phpParams + [phpActualParameter(toPhpExpr(arg), false)];
			phpNew(phpName(phpName(artifact)), phpParams);
		}
		...
		case invoke(str methodName, list[Expression] args): {
			...
			for (arg <- args)
				phpParams = phpParams + [phpActualParameter(toPhpExpr(arg), false)];
			return phpMethodCall(phpVar(phpName(phpName("this"))), phpName(phpName(methodName)), phpParams);
		}
		...
		case invoke2(Expression prev, str methodName, list[Expression] args): {
			...
			for (arg <- args)
   			...
   			return phpMethodCall(toPhpExpr(prev), phpName(phpName(methodName)), phpParams);
		}
		
		
		// Property fetch
		case fieldAccess(str name):
			return phpPropertyFetch(phpVar(phpName(phpName("this"))), phpName(phpName(name)));
			
		case fieldAccess2(Expression prev, str name):
			return phpPropertyFetch(toPhpExpr(prev), phpName(phpName(name)));
			
		case this():
			return phpVar(phpName(phpName("this")));
	};
}
...
PhpName toPhpTypeName(Type \type) {
	switch(\type) {
		...
		case float(): return phpName("float");
...
PhpStmt toPhpStmt(Statement stmt) {
	switch (stmt) {
		...
		case declare(Type t, Expression var, assign(assignable, op, expr)):
